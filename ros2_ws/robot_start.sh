...
#    --robot-host   <ip>                Robot IP (required for --role pc; also
#                                       added as a DDS peer)
#    --pc-host      <ip>                Operator PC IP (rock64 role) - added as
#                                       a DDS peer so discovery is symmetric
#    --peer         <ip>                Extra DDS unicast peer (repeatable)
...
# The peer list is generated from the hosts you pass - no manual XML editing.
_resolve_ip() {
  # Echo an IPv4 for the given host/IP, or the input unchanged if unresolved.
  local h="$1"
  [[ "$h" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]] && { echo "$h"; return; }
  local ip
  ip="$(getent hosts "$h" 2>/dev/null | awk '{print $1; exit}')"
  echo "${ip:-$h}"
}

_write_fastdds_profile() {
  # $1 = output path; remaining args = peer IPs
  local out="$1"; shift
  {
    echo '<?xml version="1.0" encoding="UTF-8" ?>'
    echo '<!-- Generated by robot_start.sh - do not edit; regenerated each launch. -->'
    echo '<dds xmlns="http://www.eprosima.com/XMLSchemas/fastRTPS_Profiles">'
    echo '  <profiles>'
    echo '    <participant profile_name="unicast_peer_discovery" is_default_profile="true">'
    echo '      <rtps><builtin>'
    echo '        <metatrafficUnicastLocatorList>'
    echo '          <locator><udpv4><address>0.0.0.0</address></udpv4></locator>'
    echo '        </metatrafficUnicastLocatorList>'
    echo '        <initialPeersList>'
    local p
    for p in "$@"; do
      echo "          <locator><udpv4><address>${p}</address></udpv4></locator>"
    done
    echo '        </initialPeersList>'
    echo '      </builtin></rtps>'
    echo '    </participant>'
...
# Build the peer set: always include loopback + this machine's LAN IP, then the
# other side (and any --peer extras). Listing the other side on at least one
# machine is enough for bidirectional unicast discovery.
_dds_peer_candidates=("127.0.0.1")
_local_ip="$(hostname -I 2>/dev/null | awk '{print $1}')"
[[ -n "$_local_ip" ]] && _dds_peer_candidates+=("$_local_ip")
if [[ "$ROLE" == "pc" ]]; then
  [[ -n "$ROBOT_HOST" ]] && _dds_peer_candidates+=("$(_resolve_ip "$ROBOT_HOST")")
else
  [[ -n "$PC_HOST" ]] && _dds_peer_candidates+=("$(_resolve_ip "$PC_HOST")")
fi
for _p in "${DDS_PEERS[@]:-}"; do
  [[ -n "$_p" ]] && _dds_peer_candidates+=("$(_resolve_ip "$_p")")
done

# Deduplicate while preserving order.
_dds_peers=()
for _p in "${_dds_peer_candidates[@]}"; do
  [[ -z "$_p" ]] && continue
  _seen=0
  for _q in "${_dds_peers[@]:-}"; do [[ "$_q" == "$_p" ]] && _seen=1 && break; done
  [[ "$_seen" -eq 0 ]] && _dds_peers+=("$_p")
done

_dds_xml="${HOME}/.ros/fastdds_unicast.generated.xml"
mkdir -p "${HOME}/.ros" 2>/dev/null || true
if _write_fastdds_profile "$_dds_xml" "${_dds_peers[@]}"; then
  export FASTRTPS_DEFAULT_PROFILES_FILE="$_dds_xml"
  info "Discovery: unicast peers [${_dds_peers[*]}] via $_dds_xml"
else
  # Fall back to the static profile shipped with the package.
  _dds_static="$(ros2 pkg prefix robot_control 2>/dev/null)/share/robot_control/config/fastdds_unicast.xml"
  if [[ -f "$_dds_static" ]]; then
    export FASTRTPS_DEFAULT_PROFILES_FILE="$_dds_static"
    warn "Could not generate DDS profile; using static $_dds_static"
  else
    warn "DDS profile unavailable - using default multicast discovery."
  fi
fi
